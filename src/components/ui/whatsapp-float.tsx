...
type WhatsAppFloatProps = {
  ...
  label?: string;
  /**
   * Visible text beside the icon. A bare green circle is recognisable but says
   * nothing about why you would tap it; on a site that sells something, saying
   * "Order honey" is the difference between an icon and an invitation. Left
   * unset the button stays a plain circle.
   */
  text?: string;
  className?: string;
};
...
export function WhatsAppFloat({
  ...
  return (
    <a
      ...
      title={label}
      className={cn(
        "fixed bottom-4 end-4 z-50 flex items-center justify-center rounded-full bg-[#25D366] text-white shadow-lg transition-transform hover:scale-105",
        /* The pill keeps the same 56px tap target as the circle; it only grows
           sideways, and only where there is room for the words.

           On a phone it stays a circle. A 270px pill fixed over a 390px screen
           covered the price table it was meant to support — a floating control
           must not sit on top of the content. The words come back at `sm`. */
        text ? "size-14 sm:h-14 sm:w-auto sm:gap-3 sm:px-6" : "size-14",
        className,
      )}
    >
